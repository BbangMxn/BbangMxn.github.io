---
title: Bbang
cssclasses:
  - portfolio-home
---

<section class="portfolio-intro">
  <p class="portfolio-kicker">Backend Portfolio</p>
  <h1>이문규의 백엔드 포트폴리오</h1>
  <p class="portfolio-lede">
    캐시, JVM, 문서 시스템을 중심으로 결과물과 연구 과정을 함께 정리합니다.
    바깥에서는 포트폴리오처럼 읽히고, 안쪽에서는 Obsidian 작업 구조가 그대로 이어지도록 설계했습니다.
  </p>
  <ul class="portfolio-meta">
    <li><strong>Focus</strong><span>Cache, JVM, Backend Architecture</span></li>
    <li><strong>Workflow</strong><span>Obsidian -> Quartz -> GitHub Pages</span></li>
    <li><strong>Format</strong><span>Portfolio landing + section hubs + working notes</span></li>
  </ul>
  <div class="portfolio-actions">
    <a class="portfolio-button primary" href="./projects">Projects</a>
    <a class="portfolio-button" href="./research">Research</a>
    <a class="portfolio-button" href="./wiki">Wiki</a>
    <a class="portfolio-button" href="./Diary">Diary</a>
  </div>
</section>

<section class="portfolio-block">
  <p class="portfolio-block-kicker">Selected Work</p>
  <h2>먼저 보면 좋은 문서</h2>
  <ul class="portfolio-list">
    <li class="portfolio-item">
      <a href="./projects/quartz-wiki-site">
        <span class="portfolio-badge">Project</span>
        <strong>Quartz 기반 개인 위키 구축</strong>
        <p>Obsidian 문서 구조, GitHub Pages 배포, 포트폴리오 UI를 한 저장소 안에서 연결한 작업입니다.</p>
      </a>
    </li>
    <li class="portfolio-item">
      <a href="./research/Cache">
        <span class="portfolio-badge">Research</span>
        <strong>Cache Research</strong>
        <p>캐시 전략과 시스템 설계를 논문형 문서 묶음으로 정리하는 리서치 트랙입니다.</p>
      </a>
    </li>
    <li class="portfolio-item">
      <a href="./wiki/obsidian-workflow">
        <span class="portfolio-badge">Workflow</span>
        <strong>Obsidian Workflow</strong>
        <p>작성, 링크, 발행이 실제로 어떻게 이어지는지 운영 기준으로 정리한 문서입니다.</p>
      </a>
    </li>
  </ul>
</section>

<section class="portfolio-block">
  <p class="portfolio-block-kicker">Sections</p>
  <h2>사이트는 네 개의 섹션으로 운영합니다</h2>
  <ul class="portfolio-list">
    <li class="portfolio-item">
      <a href="./projects">
        <span class="portfolio-badge">Projects</span>
        <strong>결과물과 사례</strong>
        <p>완성한 작업을 문제 정의, 구조 선택, 구현 결과 기준으로 정리합니다.</p>
      </a>
    </li>
    <li class="portfolio-item">
      <a href="./research">
        <span class="portfolio-badge">Research</span>
        <strong>실험과 비교 메모</strong>
        <p>아직 진행 중인 주제를 빠르게 기록하고, 구조가 잡히면 프로젝트나 위키로 옮깁니다.</p>
      </a>
    </li>
    <li class="portfolio-item">
      <a href="./wiki">
        <span class="portfolio-badge">Wiki</span>
        <strong>운영 기준과 지식 문서</strong>
        <p>반복해서 보는 개념과 배포 기준을 오래 유지하는 문서 섹션입니다.</p>
      </a>
    </li>
    <li class="portfolio-item">
      <a href="./Diary">
        <span class="portfolio-badge">Diary</span>
        <strong>공개 가능한 기록</strong>
        <p>Obsidian vault의 메모 중 공개 가능한 개인 기록만 별도로 정리합니다.</p>
      </a>
    </li>
  </ul>
</section>

<section class="portfolio-block">
  <p class="portfolio-block-kicker">Info</p>
  <h2>처음 방문했다면 여기부터 보면 됩니다</h2>
  <ul class="portfolio-list">
    <li class="portfolio-item">
      <a href="./start-here">
        <span class="portfolio-badge">Guide</span>
        <strong>Start Here</strong>
        <p>사이트를 어떤 순서로 읽으면 되는지 빠르게 안내하는 시작 문서입니다.</p>
      </a>
    </li>
    <li class="portfolio-item">
      <a href="./about">
        <span class="portfolio-badge">About</span>
        <strong>About</strong>
        <p>이 사이트를 왜 만들었고 어떤 구조로 운영하는지 설명하는 소개 문서입니다.</p>
      </a>
    </li>
  </ul>
</section>
