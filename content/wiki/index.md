---
title: Wiki
tags:
  - wiki
cssclasses:
  - folder-hub
---

<div class="hub-header">
  <p class="hub-kicker">Wiki</p>
  <h2>반복해서 참조할 개념과 운영 기준을 정리하는 섹션입니다</h2>
  <p class="hub-lede">
    Wiki는 작업이 끝난 뒤에도 계속 참고할 문서를 모아두는 공간입니다.
    Research에서 정리된 내용을 더 안정적인 형태로 다듬어 이곳에 남깁니다.
  </p>
</div>

<div class="hub-grid">
  <a class="hub-card" href="./github-pages">
    <span class="hub-label">Deploy</span>
    <strong>GitHub Pages</strong>
    <p>현재 사이트의 배포 구조와 운영 시 기억해야 할 기준을 정리한 문서입니다.</p>
  </a>
  <a class="hub-card" href="./obsidian-workflow">
    <span class="hub-label">Workflow</span>
    <strong>Obsidian Workflow</strong>
    <p>문서 작성에서 링크, 미리보기, 퍼블리시까지 이어지는 실제 작성 흐름을 설명합니다.</p>
  </a>
  <a class="hub-card" href="./Java">
    <span class="hub-label">Language</span>
    <strong>Java Wiki</strong>
    <p>Java 런타임 리서치에 연결되는 기본 개념과 반복 참조 문서를 모아둡니다.</p>
  </a>
</div>
