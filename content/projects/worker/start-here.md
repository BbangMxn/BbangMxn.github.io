...
4. [Proposal Safety](./proposal-safety)
...
- `worker_client`와 `worker_server`를 어떤 순서로 읽어야 하는지, 서버 안에서는 `api/worker mode`와 헥사고날 계층을 어떻게 봐야 하는지 설명합니다.
- 복잡한 저장소를 읽기 위한 코드 기준 입구 문서입니다.
...
- 실제 폴더와 프로세스 경계부터 잡고 싶다면 [Architecture](./architecture)을 먼저 보는 편이 더 빠릅니다.
