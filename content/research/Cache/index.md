...
Cache, Eviction Policy, 3-Tier Caching, Redis, Benchmark, Throughput, Cache Invalidation
...
| 항목 | 관찰 |
| --- | --- |
| 구조 | L1 In-Memory, L2 Redis, L3 Query Cache로 계층화할 때 응답 시간과 공유성의 균형이 좋아진다. |
| 벤치마크 | 14개 알고리즘 평균 히트율 차이는 `42.96% ~ 44.60%`로 크지 않았다. |
| 실무 판단 | 히트율 1~2%p보다 스루풋, 메모리 오버헤드, 구현 난이도가 더 중요하다. |
| 특이점 | `CLOCK`은 Java 환경에서만 유의미하게 빠른 결과를 보였다. |
| 향후 방향 | LeCaR 같은 경량 적응형 기법부터 시작해 예측 기반 캐싱으로 확장하는 경로가 현실적이다. |

## 문서 구성

- [[research/Cache/01-research-purpose-and-questions|1. 연구 배경과 목적]]
- [[research/Cache/02-cache-taxonomy-and-system-design|2. 캐시 구조와 시스템 설계]]
- [[research/Cache/03-benchmark-method-and-results|3. 벤치마크 설계와 결과]]
- [[research/Cache/04-discussion-and-implications|4. 해석과 실무 시사점]]
- [[research/Cache/05-future-work|5. 향후 연구 방향]]

## 읽기 안내

빠르게 훑으려면 1번과 4번만 읽어도 된다. 실제 구조 설계와 근거가 필요하면 2번과 3번을, 연구 확장 방향까지 보려면 5번을 이어서 읽으면 된다.
