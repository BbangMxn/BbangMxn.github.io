---
title: Java Object Research
tags:
  - research
  - java
  - jvm
cssclasses:
  - folder-hub
---

<div class="hub-header">
  <p class="hub-kicker">Research / Java</p>
  <h2>Java 객체와 JVM 구조를 연구합니다</h2>
  <p class="hub-lede">
    문법보다 객체 모델, 메모리 비용, 런타임 구조 같은 시스템 관점에 집중합니다.
  </p>
</div>

<section class="hub-section">
  <p class="hub-section-kicker">Browse</p>
  <h3>허브와 시작 문서</h3>
  <ul class="hub-list">
    <li class="hub-item">
      <a href="./Java Object">
        <span class="hub-label">Folder</span>
        <strong>Java Object</strong>
        <p>JVM 구조와 객체 오버헤드 문서를 모아둔 허브입니다.</p>
      </a>
    </li>
    <li class="hub-item">
      <a href="./Java Object/00 Index">
        <span class="hub-label">Entry</span>
        <strong>00 Index</strong>
        <p>연구 흐름과 읽는 순서를 먼저 확인하는 시작 문서입니다.</p>
      </a>
    </li>
  </ul>
</section>
